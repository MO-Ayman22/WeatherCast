...
enum WeatherDetailsState: Equatable {
    ...
    static func == (
        lhs: WeatherDetailsState,
        rhs: WeatherDetailsState
    ) -> Bool {
...
final class WeatherDetailsViewModel: ObservableObject {
    ...
    @Published var state: WeatherDetailsState = .idle
    ...
    init(
        ...
        self.getWeatherForLocationUseCase =
            container.getWeatherForLocationUseCase

        self.saveLocationUseCase =
            container.saveLocationUseCase

        self.removeLocationUseCase =
            container.removeLocationUseCase

        self.checkIfSavedUseCase =
            container.checkIfSavedUseCase
    }
    ...
    func loadWeather() {
        ...
        getWeatherForLocationUseCase
            .execute(query: city.queryString)
    ...
    func toggleSave() {
        ...
        guard case .success(let bundle) = state
        else { return }
    ...
    private func checkIfSaved(
        ...
        checkIfSavedUseCase
            .execute(locationName: locationName)
            .receive(on: DispatchQueue.main)
            .sink { _ in
